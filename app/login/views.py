...
def cookie(func):
    def wrapper(obj, request, *args, **kwargs):
        ...
        response.set_cookie(key='refresh-token', value=response.data['refresh'],
                            httponly=True, expires=datetime.datetime.now() + timedelta(1), path="/")
        return response
    return wrapper
...
class LoginView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        try:
            cursor = cn.cursor()
            cursor.execute(
                'insert into login_loginmodel(refresh_token, user_id_id) values(%s, %s)', (response['refresh'], user.id))
        finally:
            cursor.close()
...
def custom_token_refresh_view(request, user=None, *args, **kwargs):
    token = custom_jwt.generate_custom_token(
        user, settings.JWT_EXPIRATION_TIME, 'access')
